...
class AnchorHead(BaseDenseHead, BBoxTestMixin):
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
                    bbox_targets, bbox_weights, soft_target, num_total_samples):
        ...
        if soft_target is not None:
            soft_target = soft_target.permute(0, 2, 3,
                                              1).reshape(-1, self.cls_out_channels)
            bg_class_ind = self.num_classes
            pos_inds = ((labels >= 0)
                        & (labels < bg_class_ind)).nonzero().squeeze(1)

            return loss_cls, loss_bbox, cls_score[pos_inds], soft_target[pos_inds]
    ...
    def loss(self,
        ...
        if soft_target is None:
            soft_target = [None for _ in range(len(cls_scores))]
        losses_cls, losses_bbox, soft_stu, soft_tea = multi_apply(
            self.loss_single,
            cls_scores,
            bbox_preds,
            all_anchor_list,
        ...
        if return_sampling_results:
            return dict(loss_cls=losses_cls, loss_bbox=losses_bbox), soft_stu, soft_tea, sampling_results
        else:
    ...
    def get_bboxes(self,
        ...
        if not with_backward:
            ...
            cls_score_list = [cls_scores[i] for i in range(num_levels)]
            bbox_pred_list = [bbox_preds[i] for i in range(num_levels)]
