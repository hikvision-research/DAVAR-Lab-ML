...
#
#
# Abstract: the logger hook for knowledge distillation.
"""
import datetime
...
from .base_kd import LoggerHookKD


@HOOKS.register_module()
class TextLoggerHookKD(LoggerHookKD):

    def __init__(self, interval=10, ignore_last=True, reset_flag=False):
        super(TextLoggerHookKD, self).__init__(interval, ignore_last, reset_flag)
        self.time_sec_tot = 0

    def before_run(self, runner):
        super(TextLoggerHookKD, self).before_run(runner)
    ...
    def _log_info(self, log_dict, runner):
        if runner.mode == 'train':
            log_str = 'Epoch [{}][{}/{}]\ttrain_branch: {}, lr: {:.5f}, '.format(
                log_dict['epoch'], log_dict['iter'], len(runner.data_loader),
                log_dict['train_branch'], log_dict['lr'])
            if 'time' in log_dict.keys():
                ...
                log_str += 'time: {:.3f}, '.format(log_dict['time'])
    ...
    def log(self, runner):
        ...
        current_branch = runner.current_forward_branch()
        mode = 'train' if 'time' in runner.log_buffer.output else 'val'
        log_dict['mode'] = mode
        log_dict['epoch'] = runner.epoch + 1
        log_dict['train_branch'] = current_branch
        log_dict['iter'] = runner.inner_iter + 1
        # only record lr of the first param group
        lr_tmp = runner.current_lr()
        log_dict['lr'] = lr_tmp[current_branch][0]
        if mode == 'train':
        ...
        # for name, val in runner.log_buffer.output_probe.items():
        #     log_dict[name] = val
